...
func main() {
	...
	// Generate keys including rotation keys for packed computation
	fmt.Println("🔑 Generating keys (with rotation support)...")
	...
	fmt.Printf("✅ Keys generated in %.2fms (including rotation keys)\n\n", float64(keygenTime.Microseconds())/1000.0)
	...
	for i, tc := range testCases {
		...
		// ✨ KEY DIFFERENCE: Encrypt all features in ONE ciphertext (vector packing)
		fmt.Println("🔒 Encrypting feature vector (PACKED)...")
		startEncrypt := time.Now()

		values := make([]complex128, params.MaxSlots())
		for j, value := range features {
			values[j] = complex(value, 0)
		}
		...
		encryptedVector := base64.StdEncoding.EncodeToString(ctBytes)

		encryptTime := time.Since(startEncrypt)
		totalEncryptTime += encryptTime
		totalCiphertextSize += int64(len(ctBytes))

		fmt.Printf("✅ Packed encryption: %d features → 1 ciphertext\n", len(features))
		fmt.Printf("   Size: %.2f KB, Level=%d, Time: %.2fms\n\n",
			float64(len(ctBytes))/1024.0, ct.Level(), float64(encryptTime.Microseconds())/1000.0)

		// Send to backend (packed endpoint)
		fmt.Println("📡 Sending packed vector to backend...")
		...
		resp, err := http.Post("http://localhost:8080/api/inference-packed",
		...
		fmt.Printf("✅ Backend (packed) inference: %.2fms (Request: %.2f KB)\n\n",
		...
		fmt.Printf("✅ Decryption: %.2fms (Response: %.2f KB)\n",
	...
	if passCount > 0 {
		fmt.Println("\n" + strings.Repeat("=", 60))
		fmt.Println("📊 Aggregate Performance Metrics (PACKED OPTIMIZATION)")
		fmt.Println(strings.Repeat("=", 60))
		fmt.Printf("Key Generation:     %.2fms\n", float64(keygenTime.Microseconds())/1000.0)
		fmt.Printf("Avg Encryption:     %.2fms per test (5 features → 1 ciphertext)\n",
		...
		fmt.Println(strings.Repeat("-", 60))
		fmt.Printf("Avg Ciphertext Size: %.2f KB per test (1 packed vector)\n",
