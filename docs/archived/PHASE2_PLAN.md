# Phase 2: React 프론트엔드 개발 계획

**Status**: ✅ **COMPLETED** (November 28, 2025)

## 📋 개발 순서

### Step 1: 프로젝트 구조 설정
```
frontend/
├── public/
│   ├── main.wasm          # Lattigo Wasm 모듈
│   ├── wasm_exec.js       # Go Wasm 실행 스크립트
│   └── index.html
├── src/
│   ├── services/
│   │   ├── wasmLoader.js       # Wasm 모듈 로더
│   │   ├── cryptoService.js    # Web Crypto API 래퍼
│   │   └── indexedDBService.js # IndexedDB 헬퍼
│   ├── components/
│   │   ├── KeyManagement.jsx   # 키 생성/관리 UI
│   │   ├── EncryptPanel.jsx    # 암호화 UI
│   │   ├── DecryptPanel.jsx    # 복호화 UI
│   │   └── OperationsPanel.jsx # FHE 연산 UI
│   ├── contexts/
│   │   └── FHEContext.jsx      # FHE 상태 관리
│   ├── App.js
│   └── index.js
```

### Step 2: Wasm 로더 서비스 (wasmLoader.js)
- Go Wasm 모듈 비동기 로딩
- JavaScript 전역 함수 노출 확인
- 로딩 상태 관리

### Step 3: Web Crypto API 서비스 (cryptoService.js)
- `deriveKey(password, salt)`: PBKDF2로 AES 키 유도
- `encryptData(data, password)`: sk를 AES-GCM으로 암호화
- `decryptData(encryptedData, password)`: 암호화된 sk 복호화
- Salt 생성 및 저장

### Step 4: IndexedDB 서비스 (indexedDBService.js)
- DB 초기화 (`fhe-keys` 스토어)
- `saveEncryptedKey(keyId, encryptedData, salt)`
- `loadEncryptedKey(keyId)`
- `deleteKey(keyId)`
- `listKeys()`

### Step 5: FHE Context (상태 관리)
- Wasm 로드 상태
- 현재 키 상태 (pk, sk 존재 여부)
- 암호문 저장소 (메모리)
- FHE 함수 래퍼

### Step 6: UI 컴포넌트
1. **KeyManagement**: 키 생성, 비밀번호 설정, 키 저장/로드
2. **EncryptPanel**: 숫자 입력 → 암호화 → 암호문 표시
3. **DecryptPanel**: 암호문 선택 → 비밀번호 입력 → 복호화
4. **OperationsPanel**: 덧셈/곱셈/회전/켤레 연산

## 🔐 보안 요구사항

### sk 관리
- ✅ IndexedDB에 **암호화된 형태**로만 저장
- ✅ 메모리에서 sk는 **사용 직후 즉시 제거**
- ✅ 비밀번호는 **절대 저장하지 않음** (사용자가 매번 입력)
- ✅ Idle timeout 구현 (5분 후 자동 로그아웃)

### Web Crypto API 설정
- **PBKDF2**: 
  - Iterations: 100,000 이상
  - Hash: SHA-256
  - Salt: 16 bytes (crypto.getRandomValues)
- **AES-GCM**:
  - Key length: 256 bits
  - IV: 12 bytes (매번 새로 생성)

## 🎨 UI/UX 설계

### 메인 화면 레이아웃
```
┌─────────────────────────────────────────┐
│  🔒 Lattigo-Wasm FHE Demo               │
├─────────────────────────────────────────┤
│  [Wasm Status: Loaded ✓]                │
│  [Keys Status: SK✓ PK✓ RLK✓]           │
├─────────────────────────────────────────┤
│  Tab: [Key Management] [Encrypt]        │
│       [Decrypt] [Operations]            │
├─────────────────────────────────────────┤
│  ... 탭별 컨텐츠 ...                     │
└─────────────────────────────────────────┘
```

### Key Management 탭
- 새 키 생성 버튼
- 비밀번호 입력 필드
- 키 저장/로드 버튼
- 키 상태 표시 (크기, 생성 시간)

### Encrypt 탭
- 숫자 입력 필드
- 암호화 버튼
- 암호문 출력 (Base64)

---

## ✅ Implementation Status

### Completed Features
- ✅ **WASM Loader**: Async loading with cache busting
- ✅ **Web Crypto API**: PBKDF2 (100k iterations) + AES-GCM (256-bit)
- ✅ **IndexedDB Service**: Key storage with encryption
- ✅ **FHE Context**: React Context with state management
- ✅ **Key Management**: Generate, save, load keys with password
- ✅ **Credit Input Form**: 4-field form with preprocessing
- ✅ **Encryption**: 5 features encrypted client-side
- ✅ **RLK Generation**: Client generates RLK, sends to backend
- ✅ **Decryption**: Results decrypted client-side
- ✅ **Security**: 5-minute idle timeout, memory clearing
- ✅ **UI/UX**: Modern fintech design with CAU colors
- ✅ **Benchmark Panel**: Sequential vs Parallel testing

### Performance Metrics (E2E)
- **Key Generation**: 55.85ms
- **Encryption** (5 features): 67.35ms (13.47ms/feature)
- **Backend Inference**: 259.58ms
- **Decryption**: 8.65ms
- **Total E2E**: ~336ms

### Test Results
- **E2E Tests**: 5/5 passing (100%)
- **Web Workers**: 1.0x speedup (not beneficial)
- **Decryption Accuracy**: Valid scores in [0,1] range
- 암호문 크기 표시
- 암호문 목록 (메모리)

### Decrypt 탭
- 암호문 선택 드롭다운
- 비밀번호 입력
- 복호화 버튼
- 결과 표시

### Operations 탭
- 연산 선택: Add / Multiply / Rotate / Conjugate
- 암호문 선택 (1개 또는 2개)
- 실행 버튼
- 결과 암호문 생성
- 성능 측정

## 📦 필요한 npm 패키지
- React (기본 포함)
- Material-UI 또는 Tailwind CSS (UI 라이브러리)
- `idb` (IndexedDB 래퍼, 선택)

## 🧪 테스트 시나리오
1. Wasm 모듈 로딩 테스트
2. 키 생성 → 저장 → 로드 플로우
3. 암호화 → 복호화 정확성
4. FHE 연산 정확성
5. 비밀번호 오류 처리
6. Idle timeout 동작
