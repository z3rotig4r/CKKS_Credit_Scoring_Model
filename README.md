...
WEB Demo Version Link: [CKKS_Credit_Scoring_based_on_WASM](https://cauckkscreditsys.netlify.app/)

---

## 🚀 빠른 시작 (Docker)

**새 PC에서 바로 실행하기 - Docker만 있으면 됩니다!**
...
```bash
# 1. 프로젝트 클론
git clone https://github.com/z3rotig4r/CKKS_Credit_Scoring_Model.git
cd CKKS_Credit_Scoring_Model

# 2. 빌드 및 실행 (첫 실행 시 약 2-3분 소요)
make quick-start
...
| 서비스 | URL | 설명 |
|--------|-----|------|
| 🌐 **프론트엔드** | http://localhost:3000 | React 웹 UI |
| 🔧 **백엔드 API** | http://localhost:8080 | Go 서버 |
| 💊 **헬스체크** | http://localhost:8080/health | 서버 상태 확인 |

### ⚠️ 브라우저가 파일을 다운로드하는 경우
브라우저 캐시 문제입니다. 다음 중 하나를 시도하세요:
1. **강제 새로고침**: `Ctrl + Shift + R` (Windows) / `Cmd + Shift + R` (Mac)
2. **시크릿 모드**: `Ctrl + Shift + N`으로 새 창 열고 접속
3. **캐시 삭제**: `Ctrl + Shift + Delete` → 캐시된 이미지 및 파일 삭제
...
> 📖 상세 가이드: [docs/DOCKER_GUIDE.md](docs/DOCKER_GUIDE.md)
